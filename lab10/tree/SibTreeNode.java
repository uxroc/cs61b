...
class SibTreeNode extends TreeNode {
  ...
  public TreeNode parent() throws InvalidNodeException {
    ...
    if(!this.valid)
		throw new InvalidNodeException("An invalid node encountered!");
	if(this.equals(this.myTree.root))
  ...
  public void insertChild(Object item, int c) throws InvalidNodeException {
	...
	if(this.firstChild==null||c==1){
		...
		SibTreeNode it = this.firstChild;
		for(int i=2;it.nextSibling!=null&&i<c;i++)
			it=it.nextSibling;
		SibTreeNode newNode = new SibTreeNode(this.myTree, item);
		newNode.parent = this;
		newNode.nextSibling = it.nextSibling;
		it.nextSibling = newNode;
	}
	this.myTree.size++;
	//System.out.println(this.myTree.toString());
  }
  ...
  public void removeLeaf() throws InvalidNodeException {
	...
	if(!this.valid)
		throw new InvalidNodeException("Not a valid node!");
	if(this.firstChild==null){
		SibTreeNode p = (SibTreeNode)this.parent();
		if(p.valid){
			SibTreeNode it=p.firstChild;
			if(!it.equals(this)){
				for(;!it.nextSibling.equals(this);it=it.nextSibling);
				it.nextSibling=this.nextSibling;
			}else{
				p.firstChild=it.nextSibling;
			}
		}
		this.parent=null;
		this.valid=false;
