...
public class SList {
  ...
  public void squish() {
	...
	SListNode iterNode1 = head;
	if (head == null) return;
	SListNode iterNode2 = iterNode1.next;
	for(; iterNode2 != null; iterNode2 = iterNode2.next) {
		if (iterNode2.item.equals(iterNode1.item)) size--;
		else {
			iterNode1.next = iterNode2;
  ...
  public void twin() {
	...
	SListNode iterNode = head;
	for(; iterNode != null; iterNode = iterNode.next.next) {
		SListNode dupNode = new SListNode(iterNode.item);
		dupNode.next = iterNode.next;
		iterNode.next = dupNode;
	}
  }
