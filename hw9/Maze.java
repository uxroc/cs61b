...
public class Maze {
  ...
  public Maze(int horizontalSize, int verticalSize) {
     ...
     **/
    int si = 2*horiz*vert-horiz-vert;

    int[] u = new int[si];
    int[] v = new int[si];
    int[] hv = new int[si];

    int[] permu = new int[si];

    int cnt=0;
    for(int a=0;a<2;a++){
	    for(int b=0;b<horiz-(a+1)%2;b++){
		    for(int c=0;c<vert-a%2;c++){
			    u[cnt]=b;
			    v[cnt]=c;
			    hv[cnt]=a;
			    cnt++;
    ...
    for(i=0;i<si-1;i++){
	    int g = randInt(si-1-i);
	    int tmp=permu[g];
	    permu[g]=permu[si-1-i];
	    permu[si-1-i]=tmp;
    }

    DisjointSets ds = new DisjointSets(vert*horiz);

    for(j=0;j<si;j++){
	    i = permu[j];
	    int w1=u[i]*vert+v[i];
	    int w2=hv[i]==1?(u[i]*vert+v[i]+1):((u[i]+1)*vert+v[i]);

	    int p1=ds.find(w1), p2=ds.find(w2);
	    if(p1!=p2){
		    if(hv[i]==1){
			    hWalls[u[i]][v[i]]=false;
		    }else{
			    vWalls[u[i]][v[i]]=false;
		    }
		    ds.union(p1,p2);
	    }
    }
