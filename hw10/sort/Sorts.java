...
public class Sorts {
  ...
  public static int[] countingSort(int[] keys, int whichDigit) {
    ...
    int base = 1;
    int[] val = new int[keys.length];
    for(int i=0;i<keys.length;i++){
	    val[i]=(keys[i]>>(4*whichDigit))&((1<<4)-1);
	    count[val[i]]++;
    }
    int c=0;
  ...
  public static int[] radixSort(int[] keys) {
    ...
    int[][] ret = new int[keys.length][8];
    for(int i=0;i<8;i++){
	if(i==0){
    		ret[i] = countingSort(keys,i);
	}else{
		ret[i]=countingSort(ret[i-1],i);
	}
    }
    return ret[7];
  }
