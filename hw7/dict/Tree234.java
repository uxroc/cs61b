...
public class Tree234 extends IntDictionary {
	...
	public void insert(int key) {
		...
		while (node != null) {
			...
			if (node != null && node.keys == 3) {
				Tree234Node c1 = new Tree234Node(p, node.key1);
				c1.child1 = node.child1;
				c1.child2 = node.child2;

				Tree234Node c2 = new Tree234Node(p, node.key3);
			...
			if (key < node.key1) {
				node = node.child1;
			} else if ((node.keys == 1 && key > node.key1) || (node.keys == 2 && key > node.key1 && key < node.key2)) {
				node = node.child2;
