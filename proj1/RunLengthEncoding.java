...
public class RunLengthEncoding implements Iterable {
  ...
  public RunLengthEncoding(int width, int height) {
    // Your solution here.
	head = new Run(width * height, 0, 0, 0);
	tail = head;
	this.width = width;
	this.height = height;
  }
  ...
  public RunLengthEncoding(int width, int height, int[] red, int[] green,
    ...
    // Your solution here.
	int l = red.length;
	head = new Run(runLengths[0], red[0], green[0], blue[0]);
	tail = head;
	for(int i = 1; i < l; i++) {
		tail.next = new Run(runLength[i], red[i], green[i], blue[i]);
		tail.next.prev = tail;
		tail = tail.next;
	}
	this.width = width;
