...
public class BinaryTree implements Dictionary {
	...
	private void removeHelper(BinaryTreeNode node, BinaryTreeNode newNode) {
		if (newNode != null) {
	...
	public Entry remove(Object key) {
		...
		if (node.rightChild == null) {
			this.removeHelper(node, node.leftChild);
		} else if (node.leftChild == null) {
			this.removeHelper(node, node.rightChild);
		} else {
			BinaryTreeNode newNode = node.rightChild;
			for (; newNode.leftChild != null; newNode = newNode.leftChild);
			if (newNode == node.rightChild) {
				newNode.leftChild = node.leftChild;
				node.leftChild.parent = newNode;
			} else {
				newNode.leftChild = node.leftChild;
				if (node.leftChild != null) node.leftChild.parent = newNode;
				newNode.parent.leftChild = newNode.rightChild;
				if (newNode.rightChild != null) newNode.rightChild.parent = newNode.parent;
				newNode.rightChild = node.rightChild;
				if (node.rightChild != null) node.rightChild.parent = newNode;
			}
			this.removeHelper(node, newNode);
		}
		this.size--;
		return ret;
	}
