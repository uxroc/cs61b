...
public class MachinePlayer extends Player {
  ...
  private static boolean atCorner(int x, int y) {
	for(int i=0;i<4;i++) {
  ...
  private static boolean atGoal(int x, int y, int color) {
	  return (atBlackGoal(x, y)&&color==BLACK)||(atWhiteGoal(x, y)&&color==WHITE);
  
  private static boolean atBlackGoal(int x, int y) {
	  return (!atCorner(x, y))&&(y==0||y==7);
  }
  
  private static boolean atWhiteGoal(int x, int y) {
  ...
  private static int countGroupChips(int x, int y, int color) {
	int ret = 1;
	visited[x][y] = 1;
	for(int i=0;i<8;i++) {
		int xx=x+fx[i], yy=y+fy[i];
		if(xx>=0&&xx<8&&yy>=0&&yy<8&&board[x][y]==color&&!visited[x][y]) {
			ret+=countGroupChips(xx, yy, color);
		}
	}
  }
  
  private static boolean isLegalMove(Move m, int color) {
  ...
  private static boolean isWinningNet(int color) {
	for(int x=0;x<8;x++)
		for(int y=0;y<8;y++)
  ...
  private static boolean dfsWinningNet(int x, int y, int color, int num) {
	  visited[x][y] = 1;
	  ...
	  for(int i=0;i<8;i++) {
		  for(int k=1;k<8;k++) {
			  ...
			  if (board[xx][yy]==color&&!visited[xx][yy]) {
